...
model = AutoModelForCausalLM.from_pretrained(model_name, mirror="modelscope", ms_dtype=mindspore.float16)
# model = MiniCPMForCausalLM.from_pretrained(model_name, mirror="modelscope")

dataset = load_dataset('csv', data_files={
    'train': './5009_nl2sql/data/nl2sql/train_dataset.csv',  # 训练数据路径， 例如'nl2sql/data/nl2sql/train_dataset.csv'
    'validation': './5009_nl2sql/data/nl2sql/val_dataset.csv'  # 验证数据路径， 例如'nl2sql/data/nl2sql/val_dataset.csv'
})
peft_output_dir = "./5009_nl2sql/peft_models/nl2sql/peft_model_ms/1"  # PEFT层的输出路径，例如'nl2sql/peft_models/nl2sql/peft_model'
...
def process_dataset(dataset, tokenizer, batch_size=6, max_seq_len=1024, shuffle=False):
    ...
    dataset = dataset.project(columns=['prompt2', 'groundtruth_sql_schema'])
    dataset = dataset.map(formatting_prompt_func, ['prompt2', 'groundtruth_sql_schema'], ['prompt'])
